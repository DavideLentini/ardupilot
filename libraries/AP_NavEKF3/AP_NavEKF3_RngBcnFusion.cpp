...
void NavEKF3_core::FuseRngBcn()
{
    ...
    if (rngPred > 0.1f)
    {
        ...
        if (rngBcnFusionReport && rngBcn.dataDelayed.beacon_ID < dal.beacon()->count()) {
            rngBcnFusionReport[rngBcn.dataDelayed.beacon_ID].beaconPosNED = rngBcnDataDelayed.beacon_posNED;
            rngBcnFusionReport[rngBcn.dataDelayed.beacon_ID].innov = rngBcn.innov;
            rngBcnFusionReport[rngBcn.dataDelayed.beacon_ID].innovVar = rngBcn.varInnov;
            rngBcnFusionReport[rngBcn.dataDelayed.beacon_ID].rng = rngBcnDataDelayed.rng;
            rngBcnFusionReport[rngBcn.dataDelayed.beacon_ID].testRatio = rngBcn.testRatio;
        }
    }
}
...
void NavEKF3_core::FuseRngBcnStatic()
{
    ...
    */
    if (!rngBcn.alignmentStarted) {
        if (rngBcn.dataDelayed.beacon_ID != rngBcn.index) {
            rngBcn.posSum += rngBcn.dataDelayed.beacon_posNED;
            rngBcn.index = rngBcn.dataDelayed.beacon_ID;
        ...
        if (rngBcn.numMeas >= 100) {
            ...
            rngBcn.receiverPosCov[2][2] = receiverPosCov[1][1] = receiverPosCov[0][0] = rngBcn.sum * tempVar;
            rngBcn.index  = 0;
            rngBcn.numMeas = 0;
            rngBcn.posSum.zero();
            rngBcn.sum = 0.0f;
    ...
    if (rngBcn.alignmentStarted) {
        ...
        if (rngBcn.alignmentCompleted) {
            ...
            if (activeHgtSource != AP_NavEKF_Source::SourceZ::BEACON) {
                ...
                ftype rngBcn.receiverPosDownMax;
                ftype rngBcn.receiverPosDownMin;
                if (delta >= 0.0f) {
                    rngBcn.receiverPosDownMax = receiverPos.z;
                    rngBcn.receiverPosDownMin = receiverPos.z - 2.0f * delta;
                } else {
                    rngBcn.receiverPosDownMax = receiverPos.z - 2.0f * delta;
                    rngBcn.receiverPosDownMin = receiverPos.z;
                }

                rngBcn.posDownOffsetMax = stateStruct.position.z - rngBcn.receiverPosDownMin;
                rngBcn.posDownOffsetMin = stateStruct.position.z - rngBcn.receiverPosDownMax;
            } else {
        ...
        ftype K_RNG[3];
        K_RNG[0] = -t35*(t19+rngBcn.receiverPosCov[0][1]*t9*t13*0.5f+receiverPosCov[0][2]*t9*t17*0.5f);
        K_RNG[1] = -t35*(t20+rngBcn.receiverPosCov[1][0]*t9*t11*0.5f+receiverPosCov[1][2]*t9*t17*0.5f);
        K_RNG[2] = -t35*(t27+rngBcn.receiverPosCov[2][0]*t9*t11*0.5f+receiverPosCov[2][1]*t9*t13*0.5f);
        ...
        if (rngBcn.health) {
            ...
            for (uint8_t i=1; i<=2; i++) {
                for (uint8_t j=0; j<=i-1; j++) {
                    ftype temp = 0.5f*(rngBcn.receiverPosCov[i][j] + receiverPosCov[j][i]);
...
void NavEKF3_core::CalcRangeBeaconPosDownOffset(ftype obsVar, Vector3F &vehiclePosNED, bool aligning)
{
    ...
    if (t8 > 0.1f) {
        ...
        rngBcn.maxOffsetStateChangeFilt = (1.0f - filtAlpha) * maxOffsetStateChangeFilt + fminF(fabsF(filtAlpha * stateChange) , 1.0f);
        ...
        if ((sq(innov) < sq(innovGateWidth) * innovVar) || aligning) {
            ...
            // covariance update
            rngBcn.posOffsetMaxVar -= gain * obsDeriv * bcnPosOffsetMaxVar;
            rngBcn.posOffsetMaxVar = MAX(bcnPosOffsetMaxVar, 0.0f);
        }
    }
    ...
    if (t8 > 0.1f) {
        ...
        // calculate a filtered state change magnitude to be used to select between the high or low offset
        ftype stateChange = innov * gain;
        rngBcn.minOffsetStateChangeFilt = (1.0f - filtAlpha) * minOffsetStateChangeFilt + fminF(fabsF(filtAlpha * stateChange) , 1.0f);
        ...
        if ((sq(innov) < sq(innovGateWidth) * innovVar) || aligning) {
            ...
            // covariance update
            rngBcn.posOffsetMinVar -= gain * obsDeriv * bcnPosOffsetMinVar;
            rngBcn.posOffsetMinVar = MAX(bcnPosOffsetMinVar, 0.0f);
        }
    }
    ...
    rngBcn.posDownOffsetMax = MAX(bcnPosDownOffsetMax, vehiclePosNED.z - bcnMidPosD + 0.5f);
    rngBcn.posDownOffsetMin  = MIN(bcnPosDownOffsetMin,  vehiclePosNED.z - bcnMidPosD - 0.5f);

    // calculate the innovation for the main filter using the offset that is most stable
    // apply hysteresis to prevent rapid switching
    if (!usingMinHypothesis && (rngBcn.minOffsetStateChangeFilt < (0.8f * rngBcn.maxOffsetStateChangeFilt))) {
        usingMinHypothesis = true;
    } else if (usingMinHypothesis && (rngBcn.maxOffsetStateChangeFilt < (0.8f * rngBcn.minOffsetStateChangeFilt))) {
